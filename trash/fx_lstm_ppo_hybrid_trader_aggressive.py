# fx_lstm_ppo_hybrid_trader_aggressive.py
#
# LSTMベース予測 × テクニカル特徴 × 攻撃的報酬 × 7段階ロット × PPO
# 「最強攻撃型」ハイブリッドFXトレーダー（研究・遊び用途限定）。
...
# PPO / RL 用
STATE_RET_LEN = 48      # 状態に入れる過去リターン数
EPISODES      = 80
STEPS_PER_EP  = 2000
...
LOSS_FACTOR        = 1.2       # 損失ペナルティ（弱め）
TREND_THRESHOLD    = 0.0001    # トレンド検出閾値（敏感）
TREND_BOOST        = 2.0       # トレンド時報酬倍率
LSTM_REWARD_SCALE  = 0.5       # LSTM予測を報酬に足す重み
...
def build_lstm_feature_feeder(prices: np.ndarray, lstm_model: LSTMReturnModel):
    ...
    def predict_next_return(t_index: int) -> float:
        # t_index: returns のインデックス（0〜len(returns)-1）
...
class HybridTradingEnv:
    ...
    def _get_state(self):
        ...
        state = np.concatenate([
            ret_window,
            np.array([vol, trend, rsi_val, lstm_pred, pos_scaled],
                     dtype=np.float32)
        ])
    ...
    def step(self, action_idx):
        ...
        # LSTM予測に沿ったポジションを取るほど加点
        lstm_pred = self.lstm_predict_func(self.t)
        reward += LSTM_REWARD_SCALE * lstm_pred * self.position
...
def train_hybrid_ppo_aggressive():
    # ---- データ準備 & LSTM学習 ----
    df = load_close_series()
    prices = df["close"].values

    print("\n=== Step 1: Train LSTM (future return predictor) ===")
    lstm_model = train_lstm(prices)
    lstm_predict_func = build_lstm_feature_feeder(prices, lstm_model)

    print("\n=== Step 2: Aggressive PPO with Hybrid Env ===")
    env = HybridTradingEnv(prices, STATE_RET_LEN, lstm_predict_func)

    state_dim = STATE_RET_LEN + 5  # ret_window + [vol, trend, rsi, lstm_pred, pos]
    ...
    for ep in range(1, EPISODES + 1):
        ...
        print(f"[Hybrid PPO Aggressive] Episode {ep}/{EPISODES} "
    ...
    plt.title("Aggressive Hybrid PPO Episode Rewards (with LSTM forecasts)")
    plt.grid()
    plt.tight_layout()
    plt.savefig("hybrid_ppo_aggressive_episode_rewards.png")
    print("Saved: hybrid_ppo_aggressive_episode_rewards.png")

    if last_equity_curve is not None:
        plt.figure(figsize=(10, 4))
        plt.plot(last_equity_curve)
        plt.title("Aggressive Hybrid PPO Trader Equity Curve (last episode)")
        plt.grid()
        plt.tight_layout()
        plt.savefig("hybrid_ppo_aggressive_equity_curve.png")
        print("Saved: hybrid_ppo_aggressive_equity_curve.png")


if __name__ == "__main__":
    print("=== Aggressive Hybrid LSTM + PPO Trader ===")
    train_hybrid_ppo_aggressive()
