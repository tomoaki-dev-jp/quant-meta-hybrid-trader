# ================================================================================
# quant_meta_hybrid_trader_v4_SOTA_FIXED.py
#
# 全部乗せ研究用トレーダーフレームワーク v4 - SOTA最新技術版（互換性修正版）
# 
# ★2024-2025最新技術★
...
#
# ※ 研究用オンリー。実運用禁止！
# ※ 修正済み版：torch.silu → F.silu（PyTorch互換性向上）
...
class ConfigV4:
    ...
    # === Mamba 設定 ===
    MAMBA_SEQ_LEN: int = 64   # シーケンス長（v3のLSTM_SEQ_LENの代替）✅ 修正済み
...
print(f"\n{'='*70}")
print(f"🚀 Quant Meta Hybrid Trader v4 - SOTA Edition (互換性修正版)")
print(f"{'='*70}")
...
class MambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x_a = F.silu(x_a)  # ✅ 修正: torch.silu → F.silu
...
class TemporalFusionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        uncertainty = F.softplus(self.uncertainty_head(x))  # ✅ 修正: torch.softplus → F.softplus
...
class LoRA_Linear(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.linear(x, w, self.bias)  # ✅ 修正: torch.nn.functional.linear
...
class ActorCriticV4(nn.Module):
    ...
    def forward(self, x):
        ...
        uncertainty = F.softplus(self.uncertainty_head(h)).squeeze(-1)  # ✅ 修正
...
def train_forecasters_v4(prices: np.ndarray):
    ...
    r = returns
    T = feat_mat.shape[0]
    seq_len = cfg.MAMBA_SEQ_LEN  # ✅ 修正済み
...
def build_feeders_v4(prices: np.ndarray, mamba_model, tft_models):
    ...
    def mamba_feeder(t: int):
        if t < cfg.MAMBA_SEQ_LEN:  # ✅ 修正済み
            return np.zeros(len(cfg.FORECAST_HORIZONS), dtype=np.float32)
        end = t + 1
        start = end - cfg.MAMBA_SEQ_LEN  # ✅ 修正済み
    ...
    def regime_feeder(t: int):
        ...
        if abs(trend) > cfg.TREND_THRESHOLD and vol < 3 * cfg.TREND_THRESHOLD:
            ...
            regime = np.array([0.1, 0.1, 0.8])
        else:
            regime = np.array([0.8, 0.1, 0.1])
...
def train_with_config_v4(envs: List[HybridEnvV4], config: Dict) -> Tuple[float, ActorCriticV4]:
    ...
    for ep in range(1, total_episodes + 1):
        ...
        old_logp_t = torch.tensor(np.array(old_logp), device=device)
